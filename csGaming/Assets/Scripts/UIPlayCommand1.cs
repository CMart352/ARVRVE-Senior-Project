...
public class UIPlayCommand1 : MonoBehaviour {
	...
	private void FixedUpdate()
	{
        ...
        if(rotated){
            StartCoroutine(Rotate()); // to be able to perform a smooth rotation
    ...
    public IEnumerator Walk()
    {
        if (path.isValidNode(indexNode)) //check if there exits a node to move to 
        {
            ...
            while (Player.transform.localPosition != currentPosition) //Vector3.Distance(Player.transform.localPosition, currentPosition) > 0.19f)
            {
                ...
                if (indexNode == 2) { //when node 1 is reached increase speed to walk between the gap
                     speed = 15f; //* Time.deltaTime;
                } else {
                    speed = 1f; //to mantain a fixed speed between the nodes
                }
